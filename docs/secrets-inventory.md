...
| `dev-new-members-channel-id` | No — different Discord server per environment | Discord channel ID where new dev-guild members receive the welcome message | Runtime | Discord Developer Portal — enable Developer Mode, then right-click the new-members channel → Copy Channel ID | Static — only changes when the new-members channel changes |
