...
| `dev-new-members-channel-id` | No — different Discord server per environment | Discord channel ID where new dev-guild members receive the welcome message | Runtime | Discord client — enable Developer Mode (User Settings → Advanced → Developer Mode), then right-click the new-members channel → Copy Channel ID | Static — only changes when the new-members channel changes |
| `prod-new-members-channel-id` | No — different Discord server per environment | Discord channel ID where new production-guild members receive the welcome message | Runtime | **Bicep-managed** (issue #299): `param newMembersChannelId` in `infra/main.bicepparam` | Static — only changes when the new-members channel changes |
