...
| `dev-app-insights-conn-string` | No — separate AI instances per environment | Azure Application Insights connection string for local dev (provisioned in PR #239, closes #182) | Runtime | Azure portal — @cbeaulieu-gt | On workspace recreation<!-- TODO(#249 follow-up): verify whether KV secret holds real conn string post-#239 or still PLACEHOLDER pending bot integration --> |
| `prod-app-insights-conn-string` | No — separate AI instances per environment | Azure Application Insights connection string for prod (provisioned in PR #239, closes #182) | Runtime | Azure portal — @cbeaulieu-gt | On workspace recreation<!-- TODO(#249 follow-up): verify whether KV secret holds real conn string post-#239 or still PLACEHOLDER pending bot integration --> |
